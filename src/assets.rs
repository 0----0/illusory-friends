...
#[async_trait]
pub trait Asset {
    async fn load(path: &Path) -> anyhow::Result<Self> where Self: Sized;
    fn delete(&self) {
...
impl<T> AssetWrapper<T> where T: Asset {
    async fn new<P>(path: P) -> anyhow::Result<Self> where P: AsRef<Path> {
...
pub struct Assets {
    ...
    // pub spritesheets: UstrMap<AssetWrapper<AnimatedSprite>>,
}
